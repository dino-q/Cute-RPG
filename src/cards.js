...
let CID = 0;
let _isEliteFn = null;
export function setCardsIsElite(fn) { _isEliteFn = fn; }
...
card("集氣珠","🔋","R","pas",["集氣+20% 大招CD-17%","集氣+40% 大招CD-29%","集氣+70% 大招CD-41%"],[s=>{s.ultRate=(s.ultRate||1)*1.2},s=>{s.ultRate=(s.ultRate||1)*1.4},s=>{s.ultRate=(s.ultRate||1)*1.7}],"#E599F7");
card("磁力場","🧲","R","atk",["大招：磁力吸引+爆發 CD12秒","範圍↑ 傷害↑ CD10秒","大範圍 高傷害 減速 CD8秒"],[s=>{s.hasMF=true;s.mfCd=12e3;s.mfR=200;s.mfDur=1500;s.mfDmgMul=1;s.mfSlow=0;s._resetUlt=true},s=>{s.hasMF=true;s.mfCd=10e3;s.mfR=260;s.mfDur=2000;s.mfDmgMul=1.5;s.mfSlow=0;s._resetUlt=true},s=>{s.hasMF=true;s.mfCd=8e3;s.mfR=320;s.mfDur=2500;s.mfDmgMul=2;s.mfSlow=1500;s._resetUlt=true}],"#339AF0");
...
card("探索靴","🗺️","R","pas",["閃避+10% 擲幣50%得寶箱","閃避+15% 擲幣50%得寶箱","閃避+20% 擲幣50%得寶箱"],[s=>{s.dodge=Math.min(1,(s.dodge||0)+.1);s._coinFlip=true},s=>{s.dodge=Math.min(1,(s.dodge||0)+.15);s._coinFlip=true},s=>{s.dodge=Math.min(1,(s.dodge||0)+.2);s._coinFlip=true}],"#12B886");
...
card("影分身","👥","SR","pas",["影分身","影分身","影分身"],[s=>{s.dblAtk=1;s._cloneCD=3000},s=>{s.dblAtk=1;s._cloneCD=3000},s=>{s.dblAtk=1;s._cloneCD=3000}],"#868E96",null,1);
...
card("黑洞","🕳️","SSR","atk",["大招變黑洞","大招黑洞 傷害+35%","大招黑洞 傷害+70%"],[s=>{s.hasBH=true;s.ultLv=1;s._resetUlt=true},s=>{s.hasBH=true;s.ultLv=2;s._resetUlt=true},s=>{s.hasBH=true;s.ultLv=3;s._resetUlt=true}],"#495057");
...
card("時間停止","⏸️","SSR","pas",["大招：凍結全敵2秒 CD15秒","大招：凍結全敵2秒 CD15秒","大招：凍結全敵2秒 CD15秒"],[s=>{s.hasTS=true;s.tsFrzDur=2e3;s.tsCd=15e3;s._resetUlt=true},s=>{s.hasTS=true;s.tsFrzDur=2e3;s.tsCd=15e3;s._resetUlt=true},s=>{s.hasTS=true;s.tsFrzDur=2e3;s.tsCd=15e3;s._resetUlt=true}],"#74C0FC",null,1);
// ═══ 劍士專屬卡 ═══
...
// ═══ 刺客專屬卡 ═══
card("致命打擊","💀","R","atk",["暴擊率+20% 暴傷+50%","暴擊率+40% 暴傷+100%","暴擊率+50% 暴傷+200%"],[s=>{s.crit=Math.min(1,(s.crit||0)+.2);s._critDmgMul=(s._critDmgMul||1)+.5},s=>{s.crit=Math.min(1,(s.crit||0)+.4);s._critDmgMul=(s._critDmgMul||1)+1},s=>{s.crit=Math.min(1,(s.crit||0)+.5);s._critDmgMul=(s._critDmgMul||1)+2}],"#DA77F2");C[C.length-1].charReq="assassin";
card("暗影步","👤","SR","pas",["攻速+20% 擊殺隱身1秒(無敵)","攻速+30% 擊殺隱身1.5秒(無敵)","攻速+50% 擊殺隱身2秒(無敵)"],[s=>{s.fr*=.8;s._shadowStep=1000},s=>{s.fr*=.7;s._shadowStep=1500},s=>{s.fr*=.5;s._shadowStep=2000}],"#9C36B5");C[C.length-1].charReq="assassin";
card("毒刃","☠️","SR","atk",["攻擊30%機率施毒+攻+3","50%施毒+攻+5","70%施毒+攻+8"],[s=>{s._poisonBlade=.3;s.atk+=3},s=>{s._poisonBlade=.5;s.atk+=5},s=>{s._poisonBlade=.7;s.atk+=8}],"#A9E34B");C[C.length-1].charReq="assassin";
card("連環殺戮","🩸","SSR","atk",["擊殺回15%HP+攻+10","擊殺回15%HP+攻+10","擊殺回15%HP+攻+10"],[s=>{s.atk+=10;s._chainKill=true;s.ls=(s.ls||0)+10},s=>{s.atk+=10;s._chainKill=true;s.ls=(s.ls||0)+10},s=>{s.atk+=10;s._chainKill=true;s.ls=(s.ls||0)+10}],"#FF4040",null,1);C[C.length-1].charReq="assassin";
card("音速投擲","🗡️","SSR","atk",["攻+10 攻速+10% 攻擊閃避20%","攻+10 攻速+20% 攻擊閃避20%","攻+10 攻速+30% 攻擊閃避20%"],[s=>{s.atk+=10;s.fr*=.9;s._atkDodge=(s._atkDodge||0)+.2},s=>{s.atk+=10;s.fr*=.8;s._atkDodge=(s._atkDodge||0)+.2},s=>{s.atk+=10;s.fr*=.7;s._atkDodge=(s._atkDodge||0)+.2}],"#74C0FC");C[C.length-1].charReq="assassin";
